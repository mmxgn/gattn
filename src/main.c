...
static void on_grid_back(GtkButton *btn, gpointer d)
{
    (void)btn; (void)d;
    toggle_grid();
}

static void toggle_grid(void)
{
    grid_toggle(app.outer_stack, app.split, &sessions,
                G_CALLBACK(on_grid_back), NULL);
...
static void on_toggle_grid(GSimpleAction *a, GVariant *p, gpointer d)
{
    (void)a; (void)p; (void)d;
    toggle_grid();
}

static void on_exit_grid(GSimpleAction *a, GVariant *p, gpointer d)
{
    (void)a; (void)p; (void)d;
    toggle_grid();
}

static void on_raise(GSimpleAction *a, GVariant *p, gpointer d)
{
    (void)a; (void)p;
...
static void on_close_session(GSimpleAction *a, GVariant *p, gpointer d)
{
    (void)a; (void)p; (void)d;
...
static void on_move_session(int delta)
{
    ...
    if (next) gtk_list_box_select_row(lb, next);
}

static void on_next_session(GSimpleAction *a, GVariant *p, gpointer d)
{
    (void)a; (void)p; (void)d;
    on_move_session(1);
}

static void on_prev_session(GSimpleAction *a, GVariant *p, gpointer d)
{
    (void)a; (void)p; (void)d;
    on_move_session(-1);
}

static void on_next_unattended(GSimpleAction *a, GVariant *p, gpointer d)
{
    (void)a; (void)p; (void)d;
    GtkListBox    *lb  = get_lb();
...
static void on_child_spawned(int child_pid, int parent_id, gpointer data)
{
    (void)data;
    /* already tracked? */
    for (int i = 0; i < sessions.count; i++)
        if (sessions.items[i].pid == child_pid) return;

    /* find parent */
    Session *parent = NULL;
    for (int i = 0; i < sessions.count; i++)
        if (sessions.items[i].id == parent_id) { parent = &sessions.items[i]; break; }
    if (!parent) return;
...
static void on_child_exited(int child_pid, int parent_id, gpointer data)
{
    (void)parent_id; (void)data;
    for (int i = 0; i < sessions.count; i++) {
...
static void on_session_cleanup(GtkWidget *term, int status, gpointer data)
{
...
static void add_session(GtkWidget *split, const char *cmd, const char *working_dir)
{
    char name[64] = "shell";
    if (cmd && *cmd) {
        char tmp[256];
    ...
    if (working_dir) recents_add(working_dir);
...
static void on_session_picked(const char *cmd, const char *dir, gpointer data)
{
...
static void on_prefs_changed(gpointer data)
{
    (void)data;
...
static void on_preferences(GSimpleAction *a, GVariant *p, gpointer d)
{
    (void)a; (void)p; (void)d;
    GtkWindow *win = gtk_application_get_active_window(GTK_APPLICATION(app.gapp));
...
static void on_about(GSimpleAction *a, GVariant *p, gpointer d)
{
    (void)a; (void)p; (void)d;
...
static void on_rename_session(GSimpleAction *a, GVariant *p, gpointer d)
{
    (void)a; (void)p; (void)d;
    Session *s = selected_session();
    GtkWindow *win = gtk_application_get_active_window(GTK_APPLICATION(app.gapp));
    if (s && win) sidebar_rename_session(s, GTK_WIDGET(win));
}

static void on_focus_sidebar(GSimpleAction *a, GVariant *p, gpointer d)
{
    (void)a; (void)p; (void)d;
...
static void on_focus_content(GSimpleAction *a, GVariant *p, gpointer d)
{
    (void)a; (void)p; (void)d;
    adw_navigation_split_view_set_show_content(ADW_NAVIGATION_SPLIT_VIEW(app.split), TRUE);
    Session *s = selected_session();
    if (s) gtk_widget_grab_focus(s->terminal);
}

static void on_show_diff(GSimpleAction *a, GVariant *p, gpointer d)
{
    (void)a; (void)p; (void)d;
    Session *s = selected_session();
    GtkWindow *win = gtk_application_get_active_window(GTK_APPLICATION(app.gapp));
    if (s && win) sidebar_show_diff(s, GTK_WIDGET(win));
}

static void on_new_session_action(GSimpleAction *a, GVariant *p, gpointer d)
{
    (void)a; (void)p; (void)d;
...
static void register_action(GActionMap *map, const char *name,
...
static void on_activate(AdwApplication *app_obj, gpointer data)
{
    ...
    AdwHeaderBar *content_hdr = g_object_get_data(G_OBJECT(app.split),
                                                  "gattn-content-header");
...
int main(int argc, char *argv[])
{
    ...
    struct { const char *action; const char *accel; } bindings[] = {
    ...
    for (size_t i = 0; i < sizeof(bindings) / sizeof(*bindings); i++) {
        ...
        gtk_application_set_accels_for_action(GTK_APPLICATION(a),
                                              bindings[i].action, accels);
    }
    /* multi-accel overrides (must come after the loop) */
    gtk_application_set_accels_for_action(GTK_APPLICATION(a), "app.next-session",
        (const char *[]){ "<Control>Tab", "<Control>Down", NULL });
    gtk_application_set_accels_for_action(GTK_APPLICATION(a), "app.prev-session",
